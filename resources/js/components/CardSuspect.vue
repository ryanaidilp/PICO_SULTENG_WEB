          ...
          <div class="flex-1 text-center">
            ...
            <div class="flex items-center mt-4 flex-column">
              <div class="w-1/2">
                ...
                <h3 class="text-3xl font-bold">{{ formatNumber(active_case) }}</h3>
                <h3 class="text-sm font-bold">({{ newCaseFormat(new_case - new_finished_case) }})</h3>
                <p class="mt-2 text-sm text-gray-600">({{ percentageFormat(activeCasePercentage) }})</p>
              </div>
              <div class="w-1/2">
                <p class="text-xs md:text-base">Selesai {{ status }}</p>
                <h3 class="text-3xl font-bold">{{ formatNumber(total_finished_case) }}</h3>
