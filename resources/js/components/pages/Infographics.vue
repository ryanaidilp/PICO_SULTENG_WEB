<template>
  <div class="w-full leading-normal text-gray-800 xl:px-0 xl:mt-8">
    <section class="m-4 mt-8">
      <vue-gallery-slideshow :images="selectedImage" :index="idx" @close="idx = null"></vue-gallery-slideshow>
      <div class="bg-white rounded-lg shadow-md">
        <div class="flex flex-col flex-wrap p-5 items-left md:p-8">
          ...
          <div class="infographic-list">
            <figure class="relative w-full" v-for="(item, index) in infographics" :key="index">
              <div class="relative overflow-hidden img-container">
                ...
                >
                  ...
                  <a class="px-2 py-2 rounded-md hover:bg-gray-800" :href="item.route">
...
export default {
  ...
  methods: {
    ...
    fetchDataInfografis() {
      ...
      axios.get("/corona/api/infografis").then(response => {
        this.infographics = response.data.data;
      });
  ...
  head() {
    const title =
      "Infografis - PICO SulTeng [Pusat Informasi COVID-19 Sulawesi Tengah]";
    return {
      title,
      meta: [
        {
